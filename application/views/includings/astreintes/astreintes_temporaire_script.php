...
<!-- <script rel="text/javascript" src="<= $root_path ?>/js/astreintes_script.js" /></script> -->
...
<script>
	...
	$(document).ready(function($) {
		...
		// console.log(rang_semaine);
			// console.log('')//something.datepicker(getDate)iso...
			//something...$.datepicker.iso...
		...
		$('#calendar_select').datepicker({
			...
			showOtherMonths: true,
	      	...
	      	onSelect: function (dateText, inst) {
            	...
            	console.log('week: '+$.datepicker.iso8601Week(datepicked));
		...
		var calculateWeek = $( "#calendar_select" ).datepicker( "option", "calculateWeek" );
		console.log('calculateWeek: '+calculateWeek);

		//traitement des dates pour affichage correcte
		function date_displaying (datepicked) {
			...
			if (current_date < 7 && date_lundi < 1) {
			  ...
			  // 	  	if (date_dimanche>3) {
				...
				month_chevauche = month;//month_chevauche_avant;
		...
		function change_date_arriere (date_lundi, date_dimanche, month_chevauche_rang, mois_annee, month_rang) {
			...
			//envoie de la date a la fonction d'affichage du rang semaine
			var js_picked_date = current_year+'-'+month_rang+'-'+date_lundi;
			display_weeknumber (js_picked_date);
		}
		...
		//affichage du rang de la semaine, 
		function display_weeknumber (js_picked_date) {
			// body... 
			console.log('js_picked_date: '+js_picked_date);
			// console.log('week: '+$.datepicker.iso8601Week(js_picked_date));
			
			$.ajax({
				url: site_url+'/Astreintes/calc_week',
				method: 'POST',
				data: {js_picked_date: 'js_picked_date'},
				success: function (data) {
					 // body... 
					$('#picked_date').html(data);
				}
			});
			// $('#picked_date').html(data);					
		}
